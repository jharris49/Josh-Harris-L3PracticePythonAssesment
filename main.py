...
class Person:
    ...
    This class forms the framework for peoples inrformation to be saved as a Person object,
    including name, age, phone status, and phone number. 
    """
    def __init__(self, name, age, phone_status, phone_number = None):
        """Framework for the Person class.

        This takes the positional arguments during the creation of a Person object
        and assigns them to instance variables so the data is saved and can be used
        later on in the program.

        Parameters:
        name (str) -- the name of a person
        age (int) -- the age of a person
        phone_status (bool) -- if the person has a phone or not, true or false
        phone_number (str) -- the phone number of a person if they have one (default None)
        """
        self.name = name 
        self.age = age
...
class GetDataGUI:
    ...
    This class forms the graphical user interface for the application. 
    It holds methods that manage input validation, frame switching, person object creation,
    and widget creation and updates.
    """
    def __init__(self, parent):
        """Creates Base UI for GUI. 

        This creates instance variables for widgets and data saving. Lays down the basic UI 
        for the GUI, which can then later be adapted and changed through the methods in the
        class. 
        """
    ...
    def switch_frame(self):
        ...
        This method switches the frame shown to a user. It controls the toggle between the two frames that the program
        has. 
        """

        """Checks if the length of the list is zero 
        before letting a user switch frame by hittting the save data button. If it is zero, 
    ...
    def output_data(self, status):
        ...
        This method gets all of the data from a user chosen person object
        and outputs all of the data (instance variables) saved to that person. 

        Parameters:
        status (int) -- the status of output data, if the user wants to output the previous or next object (-1, 0, 1)
        """
    ...
    def check_entries(self):
        ...
        This method handles all input validation for widgets that take user input. Returning a bool based
        on if the entries are valid or not (valid: true, invalid: false) which is later used in an if statement. 
        It handles the UI if something is invalid and prevents invalid input from being saved in a person object. 
